...
import { SERVICES } from '@/app/preview/[templateId]/tjanster/services-data'
import type { ServiceEntry, ServiceCategory } from '@/app/preview/[templateId]/tjanster/services-data'
...
export const SITE_DEFAULTS: Record<string, SiteContent> = {
  salon: {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <p style={{ fontSize: 13, color: '#94a3b8', fontFamily: F, lineHeight: 1.6, marginBottom: 24 }}>
        Det här är din annonsyta i sökresultatet — ofta det första en ny kund läser om dig.
          ...
          <div key={ci} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            <div style={{ background: 'rgba(99,102,241,0.1)', borderBottom: '1px solid rgba(99,102,241,0.2)', padding: compact ? '10px 12px' : '14px 20px', display: 'flex', alignItems: 'center', gap: compact ? 8 : 12 }}>
              ...
              <button
                type="button"
                onClick={() => {
                  // One click must not silently erase a whole category of work
                  const n = cat.items.length
                  if (n === 0 || confirm(`Kategorin "${cat.category}" och dess ${n} tjänst${n === 1 ? '' : 'er'} tas bort. Säker?`)) {
                    removeCategory(ci)
                  }
                }}
                  ...
                  >
                    {compact && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <button
                          type="button"
                          onClick={() => { removeItem(ci, ii); setExpanded(null) }}
                          title="Ta bort tjänst"
                          style={{ background: 'none', color: '#475569', border: 'none', fontSize: 18, cursor: 'pointer', padding: 0, lineHeight: 1 }}
                        >
                          ×
                        </button>
                      </div>
                    ...
                    {!compact && (
                      <button
                        ...
                        title="Ta bort tjänst"
                        style={{ background: 'none', color: '#475569', border: 'none', fontSize: 20, cursor: 'pointer', padding: 0, lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >
                        ×
                      </button>
                    )}
