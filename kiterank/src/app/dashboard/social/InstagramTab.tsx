'use client'
import { type InstagramData } from './SocialDashboard'
...
function TypeBadge({ type }: { type: 'photo' | 'reel' }) {
  if (type === 'reel') {
    return (
  ...
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-sky-500/20 text-sky-400">
      Photo
    </span>
  )
}
...
export function InstagramTab({ data }: { data: InstagramData }) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          value={data.profileVisits.toLocaleString('sv-SE')}
          sub={`${data.profileVisitsChange > 0 ? '+' : ''}${data.profileVisitsChange}% MoM`}
          positive={data.profileVisitsChange >= 0}
        />
        <StatCard
          label="Avg. reach"
        ...
        />
        <div className="bg-navy-800 rounded-xl p-4 border border-navy-700 flex flex-col gap-1">
          <p className="text-slate-500 text-xs uppercase tracking-wider">Engagement rate</p>
          <p className="text-white text-xl font-bold leading-tight">{data.avgEngagementRate}%</p>
          <p className="text-slate-500 text-xs">industry avg. ~3%</p>
        </div>
      </div>
      ...
      <div className="bg-navy-800 rounded-xl border border-navy-700 overflow-hidden">
        ...
        <div className="grid grid-cols-[28px_80px_1fr_80px_120px_56px_64px_56px_72px] gap-3 px-5 py-2.5 border-b border-navy-700 text-xs text-slate-500 font-medium">
          <span></span>
          <span>Type</span>
          <span>Caption</span>
          <span>Date</span>
        ...
        <div className="divide-y divide-navy-700">
          {topSorted.map((post, i) => {
            ...
            const engagements = post.likes + post.comments + post.saves
            const engRate     = post.reach > 0 ? ((engagements / post.reach) * 100).toFixed(1) : '—'
            const rankColor   = RANK_COLORS[i] ?? '#475569'
            return (
              <div key={i} className="grid grid-cols-[28px_80px_1fr_80px_120px_56px_64px_56px_72px] gap-3 px-5 py-3 items-center">
                <span className="text-xs font-bold tabular-nums" style={{ color: rankColor }}>#{i + 1}</span>
                <TypeBadge type={post.type} />
                <span className="text-slate-300 text-sm truncate" title={post.caption}>{post.caption}</span>
                ...
                <span className="text-slate-300 text-xs text-right tabular-nums">{post.likes}</span>
                <span className="text-slate-300 text-xs text-right tabular-nums">{post.comments}</span>
                <span className="text-slate-300 text-xs text-right tabular-nums">{post.saves}</span>
      ...
      <div className="bg-navy-800 rounded-xl border border-navy-700 overflow-hidden">
        ...
        <div className="grid grid-cols-[80px_1fr_80px_120px_56px_64px_56px_72px] gap-3 px-5 py-2.5 border-b border-navy-700 text-xs text-slate-500 font-medium">
          <span>Type</span>
          <span>Caption</span>
          <span>Date</span>
          <span>Reach</span>
        ...
        <div className="divide-y divide-navy-700">
          {data.recentPosts.map((post, i) => {
            ...
            const engRate     = post.reach > 0 ? ((engagements / post.reach) * 100).toFixed(1) : '—'
            return (
              <div key={i} className="grid grid-cols-[80px_1fr_80px_120px_56px_64px_56px_72px] gap-3 px-5 py-3 items-center">
                <TypeBadge type={post.type} />
                <span className="text-slate-300 text-sm truncate" title={post.caption}>{post.caption}</span>
                <span className="text-slate-400 text-xs">{fmtDate(post.date)}</span>
                <div className="flex items-center gap-2">
