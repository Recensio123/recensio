...
const mockData = {
  ...
  /* The pages a site we build actually has: the start page, the treatment
     ...
     the only ones that multiply. The example used to list a plumber's
     /services/plumbing and not a single article, which is the one page type
     the customer keeps adding to. */
  pages: [
    { path: '/',                              sessions: 540, engagementRate: 0.58, avgDuration:  87, impressions: 2100, clicks: 148, position:  4.2, positionChange:  0, topQuery: 'frisör södermalm',        trend: [580, 560, 540, 510, 530, 540] },
    { path: '/tjanster',                      sessions: 232, engagementRate: 0.69, avgDuration: 164, impressions: 1480, clicks: 118, position:  5.4, positionChange:  1, topQuery: 'frisör priser stockholm', trend: [190, 198, 205, 214, 226, 232] },
    { path: '/tjanster/balayage',             sessions: 210, engagementRate: 0.71, avgDuration: 198, impressions: 1840, clicks: 132, position:  3.1, positionChange:  1, topQuery: 'balayage stockholm',      trend: [170, 180, 185, 195, 205, 210] },
    { path: '/kontakt',                       sessions: 188, engagementRate: 0.82, avgDuration: 234, impressions:  420, clicks:  41, position:  8.4, positionChange:  1, topQuery: 'frisör södermalm boka',   trend: [200, 195, 190, 185, 187, 188] },
    { path: '/tjanster/klippning-dam',        sessions: 145, engagementRate: 0.67, avgDuration: 175, impressions:  960, clicks:  78, position:  5.7, positionChange: -1, topQuery: 'klippning dam stockholm',  trend: [160, 155, 148, 142, 143, 145] },
    { path: '/artiklar/sa-haller-fargen-langre',     sessions: 128, engagementRate: 0.74, avgDuration: 262, impressions: 2600, clicks: 104, position:  6.2, positionChange:  3, topQuery: 'hur länge håller balayage', trend: [42,  58,  74,  92, 112, 128] },
    { path: '/om-oss',                        sessions:  75, engagementRate: 0.54, avgDuration: 112, impressions:  310, clicks:  22, position: 11.2, positionChange:  0, topQuery: 'salong södermalm om',     trend: [118, 108,  95,  88,  82,  75] },
    { path: '/tjanster/keratinbehandling',    sessions:  64, engagementRate: 0.74, avgDuration: 188, impressions: 1240, clicks:  58, position:  6.8, positionChange:  2, topQuery: 'keratinbehandling pris',   trend: [40,  44,  50,  56,  60,  64] },
    { path: '/artiklar/balayage-eller-slingor',      sessions:  47, engagementRate: 0.66, avgDuration: 241, impressions: 1900, clicks:  44, position:  9.8, positionChange:  1, topQuery: 'vilken hårfärg passar mig', trend: [31,  34,  38,  41,  44,  47] },
    { path: '/artiklar/torrt-har-pa-vintern',  sessions:  38, engagementRate: 0.61, avgDuration: 254, impressions: 2800, clicks:  36, position:  9.1, positionChange: -2, topQuery: 'sköta slingor hemma',      trend: [55,  50,  47,  43,  40,  38] },
    { path: '/tjanster/barnklippning',        sessions:  21, engagementRate: 0.63, avgDuration: 162, impressions: 1100, clicks:   9, position: 14.5, positionChange:  1, topQuery: 'barnklippning stockholm',   trend: [14,  16,  17,  18,  20,  21] },
  ],
}
...
export default async function AnalyticsPage() {
  ...
  const { data: company } = user
    ? await admin.from('companies').select('id, slug').eq('user_id', user.id).single()
    : { data: null }

  /* The newest snapshot for each window. The selector picks between them in
     the browser, so all three come down together — three small rows, and the
     alternative is a round trip every time someone clicks Veckovis. */
  const { data: snapshots } = company
    ? await admin
        .from('ga4_snapshots')
        .select('*')
  ...
  /* Rows come back untyped from the database and are shaped like the mock —
     the dashboard reads them through the same AnalyticsData contract. Rows
     written before the period column existed are the monthly window. */
  type Snap = typeof mockData & { period?: string | null; synced_at?: string | null }
  const rows   = (snapshots ?? []) as unknown as Snap[]
  const newest = (w: string) => rows.find(r => (r.period ?? 'Monthly') === w) ?? null
  const snapshot = newest('Monthly')
  ...
  /* Page paths are ASCII by necessity, so a name derived from one loses every
     å, ä and ö — "Så håller färgen" comes back as "Sa haller fargen". The site
     itself holds the real titles, so they are looked up here and the tab shows
     what the customer actually named the page. */
  const site = company?.slug ? await getPublishedSite(company.slug) : null
  const pageTitles: Record<string, string> = {}
  if (site) {
    for (const a of articlesOf(site))  pageTitles[`/artiklar/${a.slug}`] = a.title
    for (const s of servicesOf(site))  pageTitles[`/tjanster/${s.slug}`] = s.name
  }

  /* The last step of the funnel, and the only figure on the page that is money.
     It comes from our own booking table rather than from Google, so it is read
  ...
  /* What the selector switches between on a live account. On the example data
     there is one set, and the tabs scale it — see the note in each component.
     Null entries fall back to the monthly set rather than showing nothing. */
  const periods = isLive && snapshot
    ? {
        Weekly:  newest('Weekly')  ?? snapshot,
        Monthly: snapshot,
